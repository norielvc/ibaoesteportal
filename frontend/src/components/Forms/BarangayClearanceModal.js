...
export default function BarangayClearanceModal({ isOpen, onClose }) {
  ...
  const [referenceNumber, setReferenceNumber] = useState('BC-2026-00001');
  ...
  const fetchNextReferenceNumber = async () => {
    setLoadingRef(true);
    try {
      const response = await fetch(`${API_URL}/api/certificates/next-reference/barangay_clearance`);
      ...
      setReferenceNumber(`BC-${new Date().getFullYear()}-${timestamp}`);
    } finally {
      setLoadingRef(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        setNotification({ type: 'success', title: 'Application Submitted!', message: `Barangay Clearance (${result.referenceNumber}) has been submitted successfully. We will notify you when it's ready for pickup.` });
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-blue-800 px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-bold text-white">New Barangay Clearance Application</h2>
                <p className="text-blue-200 text-sm">Reference: {referenceNumber}</p>
              </div>
            </div>
            ...
            {!showPreview ? (
              <form onSubmit={handleSubmit} className="p-6 space-y-6">
                <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-4">
                  <div className="flex items-center gap-3">
                    <div className="bg-blue-100 p-2 rounded-lg"><Info className="w-5 h-5 text-blue-600" /></div>
                    <div>
                      <label className="block text-sm font-semibold text-blue-800">Reference Number</label>
                      <p className="text-2xl font-mono font-bold text-blue-900 mt-1">{referenceNumber}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <textarea name="purpose" value={formData.purpose} onChange={handleInputChange} rows={3}
                      placeholder="e.g., For Employment, ID Application, Business Permit, etc."
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 resize-none uppercase" />
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-5 border border-green-200">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-green-700 mt-1">We will send a message when your clearance is ready for pickup.</p>
                  </div>
                </div>
              </form>
            ) : (
              <ClearancePreview formData={formData} referenceNumber={referenceNumber} currentDate={currentDate} officials={officials} certificateRef={certificateRef} />
            )}
          </div>

          <div className="border-t bg-gray-50 px-6 py-4 flex flex-col sm:flex-row gap-3 justify-end no-print">
            <button type="button" onClick={() => setShowPreview(!showPreview)}
              className="px-6 py-2.5 border border-gray-300 rounded-lg font-medium text-gray-700 hover:bg-gray-100 flex items-center justify-center gap-2 no-print">
            ...
            {showPreview && (
              <>
                <button type="button" onClick={() => {
                  // Open print dialog for just the certificate
                  ...
                  printWindow.document.write(`
                    ...
                    <html>
                    <head>
                      <title>Barangay Clearance - ${referenceNumber}</title>
                  ...
                  className="px-6 py-2.5 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-lg font-medium flex items-center justify-center gap-2 shadow-lg no-print">
                  <Printer className="w-5 h-5" />Print
                </button>
                <button type="button" onClick={() => handleDownloadPDF()} disabled={isDownloading}
                  className="px-6 py-2.5 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-lg font-medium flex items-center justify-center gap-2 shadow-lg disabled:opacity-50 no-print">
...
// Certificate Preview - A4 Size MAXIMIZED Layout
function ClearancePreview({ formData, referenceNumber, currentDate, officials, certificateRef }) {
  ...
  return (
    <div className="p-1 flex justify-center print:p-0">
      {/* A4 Size Container - Reduced height to fit footer */}
