...
export default function ProgramsPage() {
  ...
  const saveAllChanges = async () => {
    try {
      ...
      if (data.success) {
        ...
        setNotification({
          type: "error",
          message: data.message || "Failed to save programs",
        });
      }
    } catch (error) {
      ...
      setNotification({
        type: "error",
        message: "Failed to save programs. Please try again.",
      });
    } finally {
      setSaving(false);
    }
  ...
  const addProgram = () => {
    ...
    setNotification({
      type: "success",
      message: "Program added. Remember to Save All.",
    });
  };
  ...
  const saveEdit = () => {
    ...
    setNotification({
      type: "success",
      message: 'Program updated locally. Click "Save All" to publish.',
    });
  };
  ...
  const deleteProgram = (id) => {
    ...
    setNotification({
      type: "success",
      message: 'Program removed. Click "Save All" to publish.',
    });
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Configure programs displayed on the homepage
          </p>
        </div>
          ...
          <div className="bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 p-6 flex flex-col items-center">
            {programs.length > 0 ? (
              <div className="w-full bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                ...
                />
                <div className="p-4">
                  <p className="text-indigo-600 text-[10px] font-bold uppercase mb-2">
                    {programs[0].category}
                  </p>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all resize-none"
                />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2 block">
            ...
            <div className="p-6 border-t border-gray-100 flex gap-3 shrink-0">
              <button
                onClick={saveEdit}
                className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white py-2.5 rounded-lg font-bold"
              >
                Save Changes
              </button>
              <button
                onClick={() => setEditingProgram(null)}
                className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 py-2.5 rounded-lg font-bold"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 space-y-4 overflow-y-auto flex-1">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Health Mission"
                  className="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                />
              </div>
              <div>
            ...
            <div className="p-6 border-t border-gray-100 flex gap-3 shrink-0">
              <button
                onClick={addProgram}
                className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2.5 rounded-lg font-bold"
              >
                Add Program
              </button>
              <button
...
ProgramsPage.getLayout = (page) => (
  <Layout title="Barangay Programs" subtitle="COMMUNITY INITIATIVES & SERVICES">
    {page}
  </Layout>
);
